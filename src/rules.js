...
function findLine(skill, needle, { inBody = false, wholeFile = false } = {}) {
  ...
  if (hay) {
    ...
    return idx === -1 ? undefined : bodyLine(skill, idx + 1);
  }
...
/** Convert a body-relative line number to an absolute file line number. */
function bodyLine(skill, relLine) {
  if (relLine == null) return undefined;
  return (skill.bodyOffset ?? 0) + relLine;
}

/** Scan body lines with the body→file line-number offset applied. */
function* scanBody(skill, re) {
  const offset = skill.bodyOffset ?? 0;
  for (let i = 0; i < skill.bodyLines.length; i++) {
    const m = skill.bodyLines[i].match(re);
    if (m) yield { line: offset + i + 1, match: m[0], text: skill.bodyLines[i].trim() };
  }
}
...
rule({
  ...
  run(skill) {
    const findings = [];
    // Real env access (os.environ / getenv / printenv / process.env), not
    // tool flags like `-env:UserInstallation=...`.
    const readsEnv = /\b(os\.environ|getenv|printenv|process\.env)\b|\bENV\[/;
    // Actual transfer operations, not bare URLs inside strings/XML namespaces.
    const transfers =
      /\b(curl|wget|nc\b|netcat|Invoke-WebRequest|Invoke-RestMethod|scp\b|rsync\b|requests\.(post|put)|urllib\.request\.urlopen|http\.client|fetch\(|socket\.connect)\b/i;
    for (const f of scriptFiles(skill)) {
      ...
      if (readsEnv.test(content) && transfers.test(content)) {
        findings.push({
...
rule({
  ...
  run(skill) {
    const findings = [];
    // Recursive/broad deletes only. `rm -f file` or `rm a.txt b.txt` are
    // ordinary cleanup; `rm -r/-rf` with wildcards, home paths, or `/` is not.
    const patterns = [
      [/\brm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+)+("[^"]*"|'[^']*'|[^&|;\n])*[*~]\s*(?=$|[&|;)])/, 'recursive rm over a wildcard/home path'],
      [/\brm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+)+\/(?!\w*:)\S*/, 'recursive rm from filesystem root'],
      [/\brm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+)+~(\/\S*)?/, 'recursive rm on the home directory'],
      [/\brm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+)+"?\$HOME\b/i, 'recursive rm on $HOME'],
    ...
    const scopes = [
      ['SKILL.md body', skill.bodyLines, true],
      ...scriptFiles(skill).map((f) => [`script "${f}"`, readSafe(skill, f).split('\n'), false]),
    ];
    for (const [scope, lines, isBody] of scopes) {
      for (let i = 0; i < lines.length; i++) {
        for (const [re, label] of patterns) {
          if (re.test(lines[i])) {
            findings.push({
              message: `${scope}: ${label}`,
              line: isBody ? bodyLine(skill, i + 1) : undefined,
...
rule({
  ...
  run(skill) {
    ...
    for (const hit of scanBody(skill, re)) {
...
rule({
  ...
  run(skill) {
    ...
    const looksInstructional =
      ...
      const beforeLine = bodyLine(skill, skill.body.slice(0, m.index).split('\n').length);
...
rule({
  ...
  run(skill) {
    ...
    for (const hit of scanBody(skill, linkRe)) {
